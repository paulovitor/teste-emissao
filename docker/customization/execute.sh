...
# Add MySQL module
# module add --name=com.mysql.jdbc --resources=/opt/jboss/wildfly/modules/com/mysql/jdbc/main/mysql-connector-java-5.1.31-bin.jar --dependencies=javax.api,javax.transaction.api
...
# Add the datasource
data-source add --name=MySQLDS --driver-name=mysql --jndi-name=java:jboss/datasources/MySQLDS --connection-url=jdbc:mysql://$MYSQL_URI/teste-emissao?useUnicode=true&amp;characterEncoding=UTF-8 --user-name=mysql --password=mysql --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
